...
func TestEncode(t *testing.T) {
	codec := NewMyFrameCodec()
	buf := make([]byte, 0, 128)
	rw := bytes.NewBuffer(buf) // 通过byte array创建一个缓冲区，这是一个实现了 io.Reader 和 io.Writer 接口的缓冲区。
	...
	// 验证Encode的正确性
	var totalLen int32
	err = binary.Read(rw, binary.BigEndian, &totalLen) // 读取长度放到totalLen中，长度为4字节，根据int32确定
	if err != nil {
		t.Errorf("want nil, actual %s", err.Error())
	}
